...
| Shed — local tool | A local user retrieves project context and reviews queued memory proposals before applying them. | Isolated local setup, tests, CI, design documentation, and scoped benchmarks are current. First model acquisition may require network access. | Local data can be reviewed and reset; proposed learning remains human-approved. Hosted-service operations are not applicable. | Document and reconcile the non-monotonic `v0.2.0`/`v0.3.0` release lineage in [Shed #41](https://github.com/CasterlyGit/shed/issues/41) before assigning a prospective version. |
