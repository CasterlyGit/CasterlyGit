...
| Shed — local tool | A local user retrieves project context and reviews queued memory proposals before applying them. | Isolated local setup, tests, CI, design documentation, and scoped benchmarks are current. First model acquisition may require network access. | Local data can be reviewed and reset; proposed learning remains human-approved. Hosted-service operations are not applicable. | Align public package and README version metadata to the existing `v0.3.0` release. |
| emergency-ai — source application prototype | A developer runs an offline-first PWA and optional FastAPI path to inspect jurisdiction-aware retrieval, cache, and privacy boundaries. | Supported Python CI, tests, architecture, mock-path smoke checks, and local HTTP setup are documented; no public API deployment is claimed. | Cache/offline behavior and local audit fallback are documented. Production rollback and service recovery become required only if a deployment is claimed. | Reconcile the `1.0.0` development manifest with prototype status before another release; retain clinical/legal nonvalidation limits. |
...
| Shed | `v0.3.0` is the latest release and is an ancestor of default branch; public package/README metadata still reports `0.2.0` | Align metadata to the existing release before considering another version. Continue prospective SemVer releases for verified capability milestones. | Metadata-alignment milestone first; a later `v0.4.0` requires a distinct verified capability, green supported CI, and current benchmark/setup claims. | Alignment must not move or recreate `v0.3.0`. Future notes identify setup/data effects, human-review boundaries, measured benchmark scope, migration, and recovery. Proof: tag ancestry, aligned manifest/README, tests, CI, guard, and claim audit. |
